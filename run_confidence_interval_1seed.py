...
def compare_confidence_interval(sig_level=0.1,realizations=200,varying_parameter="T",value_range=[100],seed_list=[34],verbosity=0,causal_method="PCMCI_",fit_method="LinMed"):
    ...
    df_results_est=pd.DataFrame(columns=["Estimated","Analytical"])
    ...
    for param_value in value_range:
        ...
        df_results_est_seed=pd.DataFrame(columns=["Estimated","Analytical"])
        ...
        for seed in seed_list:
            ...
            while True:
                try:
                    if rank==0: print("Seed value = %d"%random_savar.model_seed)
            ...
            #CausalMethod= PCMCI_(ts,var_names=var_names, mask=None) or AllParents
            ...
            df_results_est_seed.loc[seed,:]= [estimated_q,true_q]
    ...
    return df_results_spread,df_results_est

def plot_results(df_res_up,df_res_low,df_res_est,varying_paramater,title,save_filename):
    #Get current axis
    plt.clf()
    # Set the palette
    sns.set_palette(sns.color_palette("bright"))
    sns.lineplot(data=df_res_up)
    sns.lineplot(data=df_res_low,legend=False)
    sns.lineplot(data=df_res_est,palette=["black","grey"])
    #https://gka.github.io/palettes/#/6|d|00429d,96ffea,c6c6a9|fbff7c,df9c56,ad5a00|1|1
    # set the title
    plt.xlabel("Time sample size")
    plt.ylabel("Sensitivity")
    plt.title('%s estimate and 90%s confidence intervals '%(title,"%"))
    # show the plot
...
if __name__=="__main__":
    # Adapt causalmet, linmed in needed
    if len(sys.argv)>1:
        arg_seed = int(sys.argv[1])
    else:
        arg_seed= 12 #default seed if no argument
    run_calc=True
    varying_parameter="T"
    causalmet = "PCMCI_"
    ...
    if run_calc:
        n_seed,seed_start= 1,arg_seed
        value_range=[50,100,250,500,750,1000,2500,5000,10000,25000,50000]
        seed_list= [i for i in range(seed_start,seed_start+n_seed)]
        df_res_spread2,df_res_est2 = compare_confidence_interval(0.1,nprocs,"T",value_range,seed_list,1,causalmet,linmed)
        if rank == 0:
            df_res_low2 = copy.deepcopy(df_res_spread2)
            df_res_up2 = copy.deepcopy(df_res_spread2)
    ...
    if rank == 0:
        ...
        plot_results(df_res_up2.iloc[6:,:],df_res_low2.iloc[6:,:],df_res_est2.iloc[6:,:],varying_parameter,"Sensitivity","./plots/ci/ci_seed%d_%s_allsample.pdf"%(arg_seed,causalmet))